...
<div class="flex flex-col gap-6 md:flex-row mt-4 md:justify-between">
    <div class="flex flex-row w-full">
        <form class="w-1/2" action={{ route('pinjam.list') }}>
            <div class="flex">
                ...
                <div class="relative w-full">
                    ...
                    <div class="absolute top-0 end-0 flex h-full">
                        @if ($search)
                        <a href={{ route('pinjam.list') }} class="flex items-center pe-3">
                            <div>
...
<div id="accordion-color-body-1" class="hidden" aria-labelledby="accordion-color-heading-1">
    <div class="p-5 border border-gray-200 dark:border-gray-700 dark:bg-gray-900 rounded-lg">
        <form action={{ route('pinjam.list') }} class="flex flex-col">
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    <div class="flex items-center">
                        Title
                        <a href={{ route('pinjam.list', array_merge(['search' => $search, 'sort_by' => 'slug', 'sort' => $sort_by == 'slug' ? ($sort == 'asc' ? 'desc' : 'asc') : 'asc'], $filter)) }}>
                ...
                <th scope="col" class="px-6 py-3">
                    <div class="flex items-center">
                        Tanggal Diubah
                        <a href={{ route('pinjam.list', array_merge(['search' => $search, 'sort_by' => 'updated_at', 'sort' => $sort_by == 'updated_at' ? ($sort == 'asc' ? 'desc' : 'asc') : 'asc'], $filter)) }}>
                            <svg class="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                            </svg>
                        </a>
                ...
                <th scope="col" class="px-6 py-3">
                    <div class="flex items-center">
                        Nama Peminjam
                        <a href={{ route('pinjam.list', array_merge(['search' => $search, 'sort_by' => 'nama_peminjam', 'sort' => $sort_by == 'nama_peminjam' ? ($sort == 'asc' ? 'desc' : 'asc') : 'asc'], $filter)) }}>
                            <svg class="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                            </svg>
                        </a>
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                    <time>{{ \Carbon\Carbon::parse($buku_data->updated_at)->timezone('Asia/Jakarta')->format('d/m/Y') }}</time>
                </td>
                ...
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap ">
                    <div class="flex flex-row items-center gap-2">
                        ...
                        <a href={{ route('buku.pinjam', $buku_data->id) }}>
                            <button class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-2 text-center me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 w-full">
                                Pinjam Buku
                            </button>
                        </a>
                        @else
                        <form action={{ route('buku.selesai', ['id' => $buku_data->id]) }} method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" name="status_code" value="archieved" class="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                                Selesai Dipinjam
                            </button>
                        </form>
                        @endif
