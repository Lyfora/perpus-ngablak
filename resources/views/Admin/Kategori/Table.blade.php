        ...
        <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                ...
                <th scope="col" class="px-6 py-3">
                    <div class="flex items-center">
                        Kategori
                        <a href={{ route('kategori.list', array_merge(['search' => $search, 'sort_by' => 'name', 'sort' => $sort_by == 'slug' ? ($sort == 'asc' ? 'desc' : 'asc') : 'asc'], $filter)) }}>
                ...
                <th scope="col" class="px-6 py-3">
                    <div class="flex items-center">
                        Tanggal Dibuat
                        <a href={{ route('kategori.list', array_merge(['search' => $search, 'sort_by' => 'created_at', 'sort' => $sort_by == 'created_at' ? ($sort == 'asc' ? 'desc' : 'asc') : 'asc'], $filter)) }}>
                            <svg class="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z" />
                            </svg>
                        </a>
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    <img src={{ $kategori_data->image }} alt="" class="rounded-lg object-cover w-32 h-24">
                </th>
                <td class="px-6 py-4">
                    {{$kategori_data->name}}
                </td>
                <td class="px-6 py-4">
                    <time>{{ \Carbon\Carbon::parse($kategori_data->created_at)->timezone('Asia/Jakarta')->format('d/m/Y') }}</time>
                </td>
                <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap ">
                    <div class="flex flex-row items-center gap-2">
                        <a href={{ route('kategori.edit', $kategori_data->id) }}>
                        ...
                        <a href={{ route('kategori.detail', $kategori_data->id) }}>
                        ...
                        <form action={{ route('kategori.delete', $kategori_data->id) }} method="POST">
                            @csrf
                            @method('DELETE')
...
<nav aria-label="Page navigation example" class="mt-3">
    <ul class="flex items-center -space-x-px h-8 text-sm">
        <li>
            <a href={{ route('kategori.list', array_merge(['page' => $page == 1 ? $page : $page - 1, 'search' => $search, 'sort_by' => $sort_by, 'sort' => $sort], $filter)) }} class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
        ...
        @for ($i = 1; $i <= $kategori->lastPage(); $i++)
            ...
            <li>
                <a href={{ route('kategori.list', array_merge(['page' => $i, 'search' => $search, 'sort_by' => $sort_by, 'sort' => $sort], $filter)) }} class="flex items-center justify-center px-3 h-8 leading-tight text-blue-600 bg-blue-50 border border-blue-300 dark:border-gray-700 dark:bg-gray-700 dark:text-white">{{ $i }}</a>
            </li>
            @else
            <li>
                <a href={{ route('kategori.list', array_merge(['page' => $i, 'search' => $search, 'sort_by' => $sort_by, 'sort' => $sort], $filter)) }} class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">{{ $i }}</a>
            </li>
            @endif
            @endfor
            <li>
                <a href={{ route('kategori.list', array_merge(['page' => $page == $kategori->lastPage() ? $page : $page + 1, 'search' => $search, 'sort_by' => $sort_by, 'sort' => $sort], $filter)) }} class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
